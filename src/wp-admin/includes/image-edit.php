	...
	foreach ( $mime_transforms as $targeted_mime ) {
		if ( $targeted_mime !== $post->post_mime_type ) {
			...
			if ( 'thumbnail' === $target ) {
				...
				$result         = $img->save( $img->generate_filename( null, $thumbnail_file, $extension ) );
				...
				$result = $img->save( $img->generate_filename( null, $filename, $extension ) );
				...
				if ( method_exists( $editor, 'make_subsize' ) ) {
					foreach ( $_sizes as $size_name => $size_data ) {
						$subsized_images[ $targeted_mime ][ $size_name ] = $editor->make_subsize( $size_data );
					}
				} else {
